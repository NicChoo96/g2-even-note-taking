...
#   /            -> the G2 GLASSES app (glasses/dist) — the bare main URL the
#                    Even App prototype QR loads (same shape as the working
#                    local dev flow)
#   /app/        -> the web pasteboard app (web/dist)
...
# Web app (pasteboard UI + relay). Built with base=/app/ so its asset paths
# resolve under the /app/ route the relay serves it from.
COPY web/package.json web/package-lock.json ./web/
RUN cd web && npm ci
COPY web/ ./web/
RUN cd web && npm run build:deploy

# Glasses app (Even Hub WebView) — built with the default base (/) so it serves
