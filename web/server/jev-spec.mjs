 ...
 */
export const TOOL_KINDS = [...QUESTION_TYPES, 'rank'];
...
/**
 * @typedef {{ label: string, p: number|null, rank: number }} JevRanked
 * @typedef {{
 *   name: string,
 *   ranked: JevRanked[],
 *   top: string|null,
...
function declaredLabels(spec) {
  return spec.type === 'score' ? [...spec.criteria] : Object.keys(spec.criteria);
}
 ...
 */
function declaredOrder(name, spec, reason, confidence, first) {
 ...
 */
export function rankQuestion(name, spec, answer) {
  const confidence = typeof answer?.confidence === 'number' ? answer.confidence : null;

  if (!answer || answer.ok === false) {
    return declaredOrder(name, spec, 'no readable answer', confidence, null);
  }

  const dist = isPlainObject(answer.probabilities) ? answer.probabilities : null;
  if (!dist) {
    const claimed = spec.type === 'score' ? answer.label : answer.choice;
 ...
 * nobody asked for.
 */
export function rankAnswers(questions, answers) {
  const out = {};
  for (const [name, spec] of Object.entries(questions ?? {})) {
...
export function specFromToolArgs({ kind, question, options } = {}) {
  ...
  // `rank` is a choice read as an order: identical spec, different intent.
